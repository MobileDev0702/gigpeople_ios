...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self) {
        ...
        self.textLabel.font = [UIFont fontWithName:@"Geomanist" size:14.0f];;
        ...
        _timestampLabel.font = [UIFont fontWithName:@"Geomanist" size:12.0f];;
...
- (void)layoutSubviews
{
    ...
    CGSize labelSize =[self.textLabel.text boundingRectWithSize:CGSizeMake(self.frame.size.width-94, MAXFLOAT)
                                                     ...
                                                     attributes:@{ NSFontAttributeName:[UIFont fontWithName:@"Geomanist" size:14.0f] }
                                                        context:nil].size;
    if ([self.bubbletype isEqualToString:@"LEFT"])
    {
        CGRect textLabelFrame = self.textLabel.frame;
        textLabelFrame.origin.x = 78;
        textLabelFrame.size.width = self.frame.size.width-94;
        textLabelFrame.size.height = labelSize.height;
        textLabelFrame.origin.y = 15.0f + TOP_MARGIN;
        self.textLabel.frame = textLabelFrame;
        self.textLabel.textColor = [UIColor blackColor];
        messageBackgroundView.frame = CGRectMake(60, textLabelFrame.origin.y, labelSize.width + 30,labelSize.height + 18);
        self.AvatarImageView.frame=CGRectMake(5,10+TOP_MARGIN, 50, 50);
      
        textLabelFrame = self.textLabel.frame;
        textLabelFrame.origin.y = textLabelFrame.origin.y +8;
        self.textLabel.frame = textLabelFrame;
        
        UIImage *coloredImage = [[UIImage imageNamed:@"leftChatImage"] maskWithColor:[UIColor groupTableViewBackgroundColor]];
        messageBackgroundView.image = [UIImage imageWithCGImage:coloredImage.CGImage];
    }
    else
    {
        // Right
        ...
        messageBackgroundView.frame = CGRectMake(textLabelFrame.origin.x -8, textLabelFrame.origin.y - 2, labelSize.width + 20, labelSize.height + 18);
        self.AvatarImageView.frame=CGRectMake( self.frame.size.width-55,10+TOP_MARGIN, 50, 50);
        
        UIImage *coloredImage = [[UIImage imageNamed:@"rightChatImage"] maskWithColor:[UIColor colorWithRed:1.00 green:0.46 blue:0.00 alpha:1.0]];
        messageBackgroundView.image = [UIImage imageWithCGImage:coloredImage.CGImage];
